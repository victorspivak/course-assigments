...
abstract class TweetSet {
  ...
  def mostRetweetedAcc(accTweet:Tweet): Tweet
  ...
  def descendingByRetweet: TweetList = {
    def iter(tweets:TweetSet, sorted:TweetList): TweetList = {
      val theTweet = tweets.mostRetweeted
      val newSet = tweets.remove(theTweet)
      val newList = new Cons(theTweet, sorted)
      
      if (!newSet.isInstanceOf[Empty]) 
        iter(newSet, newList)
      else
    	newList
    }
    
    def reverseIter(toReverse:TweetList, reversed:TweetList) : TweetList = 
      if (toReverse.isEmpty) 
        reversed
      else 
        reverseIter(toReverse.tail, new Cons(toReverse.head, reversed))  
    
    reverseIter(iter(this, Nil), Nil)
  }
...
class Empty extends TweetSet {
  ...
  def mostRetweetedAcc(accTweet:Tweet): Tweet = accTweet
}
...
class NonEmpty(val elem: Tweet, val left: TweetSet, val right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    if (that.isInstanceOf[Empty]) this
	  ...
	  union(thatNonEmpty.left).union(thatNonEmpty.right).incl(thatNonEmpty.elem)
    }
  }

  def mostRetweeted: Tweet = mostRetweetedAcc(elem)

  def mostRetweetedAcc(accTweet:Tweet): Tweet = left.mostRetweetedAcc(right.mostRetweetedAcc(if (elem.retweets > accTweet.retweets) elem else accTweet))
}
