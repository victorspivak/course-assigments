...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val myServerSubscription = {
      myServer.start("/test"){r=>
        r.iterator.map{entry=>
          entry.toString()
        }
      }
    }
    ...
    val userInterrupted: Future[String] = Future.userInput("Type message: ").map(msg=> s"You entered... $msg")
    ...
    val terminationRequested: Future[String] = {
      Future.any[String](List(userInterrupted, timeOut))
    }
