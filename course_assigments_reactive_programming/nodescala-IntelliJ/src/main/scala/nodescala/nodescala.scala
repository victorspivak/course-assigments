...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val internalServerCancellation = listener.start()

    val block = async{
      while(cancellationToken.nonCancelled){
          val (request, exchange) = await{listener.nextRequest()}
          Future(respond(exchange, cancellationTokenSource.cancellationToken, handler(request)))
      }

      internalServerCancellation.unsubscribe()
    }

    Subscription(cancellationTokenSource, internalServerCancellation)
  }
}
